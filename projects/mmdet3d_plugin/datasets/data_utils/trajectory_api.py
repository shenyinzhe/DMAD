    ...
    def get_traj_label(self, sample_token, ann_tokens):
        ...
        for i, ann_token in enumerate(ann_tokens):
            ...
            if fut_traj_local.shape[0] > 0:
                ...
                fut_traj_scence_centric = convert_local_coords_to_global(fut_traj_local, trans, rot)  # misleading name, only when self.use_nonlinear_optimizer is False, it is scene centric, otherwise is box centric
            ...
            if past_traj_local.shape[0] > 0:			
                trans = np.array([0, 0, 0])		
                rot = Quaternion(matrix=box.rotation_matrix)		
                past_traj_scence_centric = convert_local_coords_to_global(past_traj_local, trans, rot)[::-1]
                ...
                if fut_traj_local.shape[0] > 0:
                    fut_traj_scence_centric = convert_local_coords_to_global(fut_traj_local, trans, rot)  # real scene centric
