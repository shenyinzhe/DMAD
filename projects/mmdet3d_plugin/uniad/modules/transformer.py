...
# - Adapt to DMAD by using forward_single_layer
...
class PerceptionTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        self.motion_decoder = build_transformer_layer_sequence(motion_decoder)
        ...
        self.track_pos_encoder = nn.Sequential(
            Linear(3, self.embed_dims),
            nn.ReLU(),
    ...
    def get_states_and_refs(
        ...
        for i in range(self.decoder.num_layers):
            # motion decoding
            motion_query, _ = self.motion_decoder.forward_single_layer(
                layer_id=i,
                query=motion_query,
                key=None,
                value=bev_embed,
            ...
            # track decoding
            query, reference_points = self.decoder.forward_single_layer(
                layer_id=i,
                query=query,
                key=None,
                value=bev_embed,
            ...
            # update query_pos
            query_pos = self.track_pos_encoder(reference_points).permute(1, 0, 2)

            # store intermediate states
            inter_states.append(query)
        ...
        return inter_states, init_reference_out, inter_references_out, motion_inter_states
