...
class UniAD(UniADTrack):
    ...
    def forward_train(self,
        ...
        losses_track, outs_track, losses_map, outs_map = self.forward_track_train(img, gt_bboxes_3d, gt_labels_3d, gt_past_traj, gt_past_traj_mask, gt_inds, gt_sdc_bbox, gt_sdc_label,
                                                        l2g_t, l2g_r_mat, img_metas, timestamp, gt_lane_labels, gt_lane_bboxes, gt_lane_masks)
        losses_track = self.loss_weighted_and_prefixed(losses_track, prefix='track')
        losses.update(losses_track)

        losses_map = self.loss_weighted_and_prefixed(losses_map, prefix='map')
        ...
        if self.with_motion_head:
            ret_dict_motion = self.motion_head.forward_train(bev_embed,
                                                        ...
                                                        outs_track=outs_track, outs_seg=outs_map
                                                    )
    ...
    def forward_test(self,
        ...
        assert rescale is True, "rescale must be True because it's hard coded True in map head"
        result = [dict() for i in range(len(img_metas))]
        result_track, result_map = self.simple_test_track(img, l2g_t, l2g_r_mat, img_metas, timestamp, gt_lane_labels, gt_lane_masks)
        ...
        if self.with_motion_head:
            result_motion, outs_motion = self.motion_head.forward_test(bev_embed, outs_track=result_track[0], outs_seg=result_map[0])
        ...
        pop_track_list = ['prev_bev', 'bev_pos', 'bev_embed', 'track_query_embeddings', 'sdc_embedding', 'motion_embeddings', 'sdc_motion_embedding']
        result_track[0] = pop_elem_in_result(result_track[0], pop_track_list)

        ### only for visualization ###
        map_thres = 0.7
        score_list = result_map[0]['pts_bbox']['score_list'].cpu().numpy().transpose([
            1, 2, 0])
        predicted_map_seg = result_map[0]['pts_bbox']['lane_score'].cpu().numpy().transpose([
            1, 2, 0])  # H, W, C
        ...
        keys_to_pop = [
            k
            for k, v in result_map[0]["pts_bbox"].items()
        ]
        for k in keys_to_pop:
            result_map[0]["pts_bbox"].pop(k, None)
        result_map[0]["pts_bbox"]["predicted_map_seg"] = predicted_map_seg
        result_map[0] = pop_elem_in_result(result_map[0], pop_list=['args_tuple'])
        ### only for visualization ###

        if self.with_seg_head:
            result_map[0] = pop_elem_in_result(result_map[0], pop_list=["pts_bbox", "args_tuple"])
        if self.with_motion_head:
