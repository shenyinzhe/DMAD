...
#
# Changes by Yinzhe Shen on 2025-04-01:
# - Adapt to DMAD
...
class UniADTrack(MVXTwoStageDetector):
    ...
    def __init__(
        ...
        self.motion_query = nn.Embedding(self.num_query+1, self.embed_dims)
    ...
    def _generate_empty_tracks(self):
        ...
        track_instances.motion_query = self.motion_query.weight

        track_instances.output_embedding = torch.zeros(
            (num_queries, dim >> 1), device=device
        )
        track_instances.output_motion_embedding = torch.zeros(
            (num_queries, dim >> 1), device=device
        )
        ...
        track_instances.mem_motion_bank = torch.zeros(
            (len(track_instances), mem_bank_len, dim // 2),
            dtype=torch.float32,
            device=device,
        )
        ...
        track_instances.coords = torch.zeros(
            (len(track_instances), 3), dtype=torch.float32, device=device
        )  # only used in evaluation to save the predicted coords
    ...
    def coordinates_ego_motion_compensation(self, coords, l2g_r1, l2g_t1, l2g_r2, l2g_t2):
        """

        Args:
            coords: obj coordinates in lidar frame, not normed
        Returns:
            ref_pts: normed, inverse sigmoid ref_pts
        """
        pc_range = self.pc_range

        ref_pts = coords @ l2g_r1 + l2g_t1 - l2g_t2
    ...
    def _forward_single_frame_train(
        ...
        det_output = self.pts_bbox_head.get_detections(
            ...
            motion_query = track_instances.motion_query,
        ...
        motion_query_feats = det_output["motion_query_feats"]
        ...
        track_instances.output_motion_embedding = motion_query_feats[-1][0]  # [300, feat_dim]

        # get the predicted coords for the next timestep
        coords = output_past_trajs[-1,0,:,self.pts_bbox_head.past_steps] + output_coords[-1, 0][..., :2]  # to box centric, i.e., lidar frame
        coords = torch.cat([coords, output_coords[-1, 0][..., 2:3]], dim=-1)  # concat z
        track_instances.coords = coords
        if l2g_r2 is not None:
            # Update predicted ref_pts with ego motion compensation
            ref_pts = self.coordinates_ego_motion_compensation(
                coords,
                l2g_r1,
                l2g_t1,
                l2g_r2,
    ...
    @auto_fp16(apply_to=("img", "prev_bev"))
    def _forward_single_frame_train_with_map(
            self,
            img,
            img_metas,
            track_instances,
        ...
        """
        Perform tracking and mapping jointly in the last frame. Called in  forward_train
        Warnning: Only Support BS=1
        Args:
            img: shape [B, num_cam, 3, H, W]
            if l2g_r2 is None or l2g_t2 is None:
        ...
        det_output, map_output = self.pts_bbox_head.get_detections_maps(
            bev_embed,
            object_query_embeds=track_instances.query,
            motion_query=track_instances.motion_query,
            ref_points=track_instances.ref_pts,
        ...
        map_loss_inputs = [
            map_output['outputs_classes'],
            map_output['outputs_coords'],
            map_output['enc_outputs_class'],
            map_output['enc_outputs_coord'],
            map_output['args_tuple'],
            map_output['reference'],
            gt_lane_labels,
            gt_lane_bboxes,
            gt_lane_masks
        ]
        map_losses = self.pts_bbox_head.map_head.loss(*map_loss_inputs, img_metas=img_metas)
        # return map_losses, map_output
        ...
        active_index = (track_instances.obj_idxes >= 0) & (track_instances.iou >= self.gt_iou_threshold) & (
        ...
        out_track_instances = self.query_interact(tmp)
        out["track_instances"] = out_track_instances
        return out, map_losses, map_output
    ...
    def select_active_track_query(self, track_instances, active_index, img_metas, with_mask=True):
        ...
        result_dict["motion_embeddings"] = track_instances.output_motion_embedding[active_index][result_dict['bbox_index']][result_dict['mask']]
        return result_dict
    ...
    def select_sdc_track_query(self, sdc_instance, img_metas):
        ...
        out["sdc_motion_embedding"] = sdc_instance.output_motion_embedding[0]
        return out
    ...
    def forward_track_train(self,
        ...
        for i in range(num_frame):
            ...
            if i != num_frame-1:  # in first frames, we do only tracking
            ...
            # in the last frame,
            else:  # in the last frame, we do tracking and mapping jointly
                frame_res, map_losses, map_output = self._forward_single_frame_train_with_map(
                    img_single,
                    img_metas_single,
                    track_instances,
                    prev_img,
        ...
        get_keys = ["bev_embed", "bev_pos", "motion_embeddings", "sdc_motion_embedding",
        ...
        return losses, out, map_losses, map_output
    ...
    def _forward_single_frame_inference_with_map(
        self,
        img,
        img_metas,
        track_instances,
        ...
        det_output, map_outs = self.pts_bbox_head.get_detections_maps(
            bev_embed,
            object_query_embeds=track_instances.query,
            motion_query=track_instances.motion_query,
            ref_points=track_instances.ref_pts,
        ...
        ### map ###
        map_bbox_list = [dict() for i in range(len(img_metas))]
        map_results = self.pts_bbox_head.map_head.get_bboxes(map_outs['outputs_classes'],
                                  map_outs['outputs_coords'],
                                  map_outs['enc_outputs_class'],
                                  map_outs['enc_outputs_coord'],
                                  map_outs['args_tuple'],
                                  map_outs['reference'],
                                  img_metas,
                                  rescale=True)
        with torch.no_grad():
            if gt_lane_masks is not None:
                drivable_pred = map_results[0]['drivable']
                ...
                lane_pred = map_results[0]['lane']
                lanes_pred = (map_results[0]['lane'].sum(0) > 0).int()
        ...
        for result_dict, pts_bbox in zip(map_bbox_list, map_results):
            result_dict['pts_bbox'] = pts_bbox
            result_dict['ret_iou'] = ret_iou
            result_dict['args_tuple'] = map_outs['args_tuple']
        ### map ###
    ...
    def simple_test_track(
        ...
        frame_res, map_res = self._forward_single_frame_inference_with_map(
            img,
            img_metas,
            track_instances,
            prev_bev,
