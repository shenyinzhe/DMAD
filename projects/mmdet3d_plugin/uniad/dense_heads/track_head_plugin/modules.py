# Copyright (c) 2023 OpenDriveLab
# Copyright (c) 2025 Yinzhe Shen
#
# Changes by Yinzhe Shen on 2025-04-01:
# - Adapt to DMAD by implementing motion query updating
...
class MemoryBank(nn.Module):
    ...
    def _build_layers(self, args, dim_in, hidden_dim, dim_out):
        ...
        self.motion_save_proj = nn.Linear(dim_in, dim_in)
        self.motion_temporal_attn = nn.MultiheadAttention(dim_in, 8, dropout=0)
        self.motion_temporal_fc1 = nn.Linear(dim_in, hidden_dim)
        self.motion_temporal_fc2 = nn.Linear(hidden_dim, dim_in)
        self.motion_temporal_norm1 = nn.LayerNorm(dim_in)
        self.motion_temporal_norm2 = nn.LayerNorm(dim_in)
    ...
    def update(self, track_instances):
        ...
        if len(saved_embed) > 0:
            ...
            motion_saved_embed = track_instances.output_motion_embedding[:, None][saved_idxes]
            motion_prev_embed = track_instances.mem_motion_bank[saved_idxes]
            motion_save_embed = self.motion_save_proj(motion_saved_embed)
            track_instances.mem_motion_bank = track_instances.mem_motion_bank.clone()
            track_instances.mem_motion_bank[saved_idxes] = torch.cat([motion_prev_embed[:, 1:], motion_save_embed], dim=1)
    ...
    def _forward_temporal_attn(self, track_instances):
        ...
        if len(embed) > 0:
            ...
            # motion query
            motion_embed = track_instances.output_motion_embedding[valid_idxes]
            prev_motion_embed = track_instances.mem_motion_bank[valid_idxes]
            motion_embed2 = self.motion_temporal_attn(
                motion_embed[None],
                prev_motion_embed.transpose(0, 1),
                prev_motion_embed.transpose(0, 1),
                key_padding_mask=key_padding_mask,
            )[0][0]
            motion_embed = self.motion_temporal_norm1(motion_embed + motion_embed2)
            motion_embed2 = self.motion_temporal_fc2(F.relu(self.motion_temporal_fc1(motion_embed)))
            motion_embed = self.motion_temporal_norm2(motion_embed + motion_embed2)
            track_instances.output_motion_embedding = track_instances.output_motion_embedding.clone()
            track_instances.output_motion_embedding[valid_idxes] = motion_embed
        return track_instances
...
class QueryInteractionModule(QueryInteractionBase):
    ...
    def _build_layers(self, args, dim_in, hidden_dim, dim_out):
        ...
        # motion query update
        self.motion_self_attn = nn.MultiheadAttention(dim_in, 8, dropout)
        self.motion_linear1 = nn.Linear(dim_in, hidden_dim)
        self.motion_dropout = nn.Dropout(dropout)
        self.motion_linear2 = nn.Linear(hidden_dim, dim_in)
        self.motion_linear_feat1 = nn.Linear(dim_in, hidden_dim)
        self.motion_linear_feat2 = nn.Linear(hidden_dim, dim_in)
        self.motion_dropout_feat1 = nn.Dropout(dropout)
        self.motion_dropout_feat2 = nn.Dropout(dropout)
        self.motion_norm_feat = nn.LayerNorm(dim_in)
        self.motion_norm1 = nn.LayerNorm(dim_in)
        self.motion_norm2 = nn.LayerNorm(dim_in)
        self.motion_dropout1 = nn.Dropout(dropout)
        self.motion_dropout2 = nn.Dropout(dropout)
    ...
    def _update_track_embedding(self, track_instances: Instances) -> Instances:
        ...
        query_pos = track_instances.query[:, :dim // 2]  # FIXME: it's the input pos, not updated output pos
    ...
    def _update_motion_embedding(self, track_instances: Instances) -> Instances:
        if len(track_instances) == 0:
            return track_instances
        dim = track_instances.query.shape[1]
        out_embed = track_instances.output_motion_embedding
        query_pos = track_instances.query[:, : dim // 2]  # FIXME: it's the input pos, not updated output pos
        query_feat = track_instances.motion_query
        q = k = query_pos + out_embed

        # attention
        tgt = out_embed
        tgt2 = self.motion_self_attn(q[:, None], k[:, None], value=tgt[:, None])[0][:, 0]
        tgt = tgt + self.motion_dropout1(tgt2)
        tgt = self.motion_norm1(tgt)

        # ffn
        tgt2 = self.motion_linear2(self.motion_dropout(self.activation(self.motion_linear1(tgt))))
        tgt = tgt + self.motion_dropout2(tgt2)
        tgt = self.motion_norm2(tgt)

        query_feat2 = self.motion_linear_feat2(
            self.motion_dropout_feat1(self.activation(self.motion_linear_feat1(tgt)))
        )
        query_feat = query_feat + self.motion_dropout_feat2(query_feat2)
        query_feat = self.motion_norm_feat(query_feat)
        track_instances.motion_query = query_feat
        # track_instances.ref_pts = inverse_sigmoid(track_instances.pred_boxes[:, :2].detach().clone())
        # update ref_pts using track_instances.pred_boxes
        return track_instances
    ...
    def forward(self, data) -> Instances:
        ...
        active_track_instances = self._update_track_embedding(
            active_track_instances)
        active_track_instances = self._update_motion_embedding(active_track_instances)
