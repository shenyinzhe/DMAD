...
class MotionTransformerDecoder(BaseModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.intention_interaction_layers = IntentionInteraction(dropout=interaction_dropout)
        self.track_agent_interaction_layers = nn.ModuleList(
            [TrackAgentInteraction(dropout=interaction_dropout) for i in range(self.num_layers)])
        self.map_interaction_layers = nn.ModuleList(
            [MapInteraction(dropout=interaction_dropout) for i in range(self.num_layers)])
