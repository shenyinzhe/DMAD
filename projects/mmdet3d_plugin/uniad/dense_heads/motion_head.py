...
# - Adapt to DMAD by renaming track_query to motion_query for better readability
...
class MotionHead(BaseMotionHead):
    ...
    def forward_train(self,
        ...
        """
        track_query = outs_track['motion_embeddings'][None, None, ...] # num_dec, B, A_track, D
        ...
        track_query = torch.cat([track_query, outs_track['sdc_motion_embedding'][None, None, None, :]], dim=2)
    ...
    def forward_test(self, bev_embed, outs_track={}, outs_seg={}):
        """Test function"""
        track_query = outs_track['motion_embeddings'][None, None, ...]
        track_boxes = outs_track['track_bbox_results']
    ...
    def forward(self,
        ...
        agent_level_anchors = self.kmeans_anchors.to(dtype).to(device).view(num_groups, self.num_anchor, self.predict_steps, 2).detach()  # in agent frame
        scene_level_ego_anchors = anchor_coordinate_transform(agent_level_anchors, track_bbox_results, with_translation_transform=True)  # B, A, G, P ,12 ,2. To ego frame, with translation
        scene_level_offset_anchors = anchor_coordinate_transform(agent_level_anchors, track_bbox_results, with_translation_transform=False)  # B, A, G, P ,12 ,2. To ego rotation frame, without translation

        # norm by the pc_range
